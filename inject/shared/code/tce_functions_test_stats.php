...
function F_getAllUsersTestStat($test_id, $group_id = 0, $user_id = 0, $startdate = 0, $enddate = 0, $full_order_field = 'total_score', $pubmode = false, $stats = 2)
{
    ...
    $sqlr = 'SELECT
		...
		user_email,
		s.subject_name,
		CAST(SUM(testlog_score) AS DECIMAL(10,2)) AS total_score,
		MAX(testlog_change_time) AS testuser_end_time
		FROM '.K_TABLE_TESTS_LOGS.', '.K_TABLE_TEST_USER.', '.K_TABLE_USERS.', '.K_TABLE_TESTS.' t, '.K_TABLE_TEST_SUBJSET.' tss, '.K_TABLE_SUBJECT_SET.' ts, '.K_TABLE_SUBJECTS.' s';
    if ($group_id > 0) {
        $sqlr .= ','.K_TABLE_USERGROUP.'';
    }
    $sqlr .= ' WHERE testlog_testuser_id=testuser_id AND testuser_user_id=user_id
                AND t.test_id = testuser_test_id
                AND t.test_id = tss.tsubset_test_id
                AND tss.tsubset_id = ts.subjset_tsubset_id
                AND ts.subjset_subject_id = s.subject_id';
    if ($pubmode) {
    ...
    $sqlr .= ' GROUP BY testuser_id, testuser_test_id, testuser_creation_time, user_id, user_lastname, user_firstname, user_name, user_email, s.subject_name, testuser_status
    ...
    if ($rr = F_db_query($sqlr, $db)) {
        ...
        while ($mr = F_db_fetch_array($rr)) {
            ...
            error_log('Debug: total_score before assignment: ' . var_export($mr['total_score'], true) . ' (Type: ' . gettype($mr['total_score']) . ')');
            $data['testuser']['\''.$mr['testuser_id'].'\'']['total_score'] = (float)$mr['total_score']; // Ensure total_score is float
            $data['testuser']['\''.$mr['testuser_id'].'\'']['subject_name'] = $mr['subject_name']; 
